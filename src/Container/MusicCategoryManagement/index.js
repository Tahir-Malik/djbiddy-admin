...
import { getMusicCategoryList, deleteMusicCategory } from '../../Redux/Actions/DashboardAction';
...
class MusicCategoryManagement extends Component {
	...
	componentDidMount() {
		// alert(1111)
		this.props.getMusicCategoryList();
	}
	DeleteUser = () => {
		let data = {
			id: this.state.deleteItem,
		};
		this.props.deleteMusicCategory(data);
	};
	DeleteModal = (id) => {
		this.setState({
	...
	componentDidUpdate(prevProps, prevState) {
		if (
			this.props.MusicCategoryListData !== prevProps.MusicCategoryListData &&
			this.props.MusicCategoryListData
		) {
			let pureval = Math.floor(this.props.MusicCategoryListData.length / 10);
			let restval = this.props.MusicCategoryListData.length % 10;
			...
			if (this.state.activePage === 0) {
				let newpagearr = [];
				for (let i = 0; i < 10; i++) {
					let newdata = { id: i, data: this.props.MusicCategoryListData[i] };
	...
	handlePageChange = (page) => {
		...
		if (page == 0) {
			...
			for (
				let i = page * 10;
				i < page * 10 + 10 && i < this.props.MusicCategoryListData.length;
				i++
			) {
				let newdata = { id: i, data: this.props.MusicCategoryListData[i] };
		...
		if (this.state.searchtype) {
			...
			if (e.target.value) {
				...
				if (this.state.searchtype === 'name') {
					newArry = this.props.MusicCategoryListData.filter((ndata, index) => {
			...
			<div className="FormAndHidden">
				...
				<div className="pulll-left-custom">
					<div className="pull-left-custom">
						<DashboardHead
							secondaryHeadingOne="  MusicCategory Management "
							secondaryHeadingTwo="  "
				...
				<div className="right__side">
					<Link
						to={{
							pathname: routeRules.addoreditMusicCategory,
							state: {
								id: '',
								MusicCategoryDetails: '',
							},
						}}
				...
				<div className="table-responsive">
					<table className="table table-borderless organic-table">
						...
						<tbody>
							{arrayNotNull(this.state.FilteredData) &&
								this.state.FilteredData.map((MusicCategory, index) => {
									if (MusicCategory.data) {
										return (
											<tr key={index}>
												<td style={{ color: '#212529' }}>{MusicCategory.id + 1}.</td>
												<td>{MusicCategory.data.music_category}</td>

												<td>
													<Link
														to="#"
														onClick={() => this.DeleteModal(MusicCategory.data.id)}
													...
													<Link
														to={{
															pathname: routeRules.addoreditMusicCategory,
															state: {
																id: MusicCategory.data.id,
																MusicCategoryDetails: MusicCategory.data,
															},
														}}
														className="detailclass"
													>
					...
					{this.state.page &&
					this.props.MusicCategoryListData &&
					this.props.MusicCategoryListData.length > 10 ? (
						<Pagination
...
const mapStateToProps = (state) => {
	...
	return {
		MusicCategoryListData: state.DashBoardReducer.MusicCategoryListData,
...
const mapDispatchToProps = (dispatch) => {
	return {
		getMusicCategoryList: () => dispatch(getMusicCategoryList()),
		deleteMusicCategory: (id) => dispatch(deleteMusicCategory(id)),
	};
};
export default connect(mapStateToProps, mapDispatchToProps)(MusicCategoryManagement);
